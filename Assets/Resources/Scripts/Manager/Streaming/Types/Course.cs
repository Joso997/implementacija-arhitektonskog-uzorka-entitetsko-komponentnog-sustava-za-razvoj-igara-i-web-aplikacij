...
namespace Manager.Streaming.Type
{
    ...
    {
        public sealed class Course : ActionInterface.Assingment
        {
            public override bool Act(IActiveObject _object, StackDataDel action = null)
            {
                ...
                InvokeController(_object, temp, GenerateSolution(temp.ToDictionary(x => x.Key, x => x.Value), operation), operation);
                return true;
            }
            ...
            public override List<int> GenerateSolution(Dictionary<string, int> dictionary, OperationEnum operation)
            {
                ...
                foreach (KeyValuePair<string, int> keyValuePair in dictionary)
                {
                    ...
                    {
                        if (i % 3 == 0)
                            temp.Add(Calculate(keyValuePair.Value, valuePair.Value, operation));
                        i++;
